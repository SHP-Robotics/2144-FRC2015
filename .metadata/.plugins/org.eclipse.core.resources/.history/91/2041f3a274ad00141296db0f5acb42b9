...
public class Robot extends SampleRobot {
		...
		public void autonomous() {
			while (isAutonomous() && isEnabled())
			{
				...
				if(numParticles > 0)
				{
					...
					scores.Aspect[0] = AspectScore(particles.elementAt(0));
					SmartDashboard.putNumber("Aspect", scores.Aspect[0]);
					scores.Area[0] = AreaScore(particles.elementAt(0));
					SmartDashboard.putNumber("Area", scores.Area[0]);
					boolean isTote = scores.Aspect[0] > SCORE_MIN && scores.Area[0] > SCORE_MIN;

					//Send distance and tote status to dashboard. The bounding rect, particularly the horizontal center (left - right) may be useful for rotating/driving towards a tote
					SmartDashboard.putBoolean("IsTote", isTote);
					SmartDashboard.putNumber("Distance", computeDistance(binaryFrame, particles.elementAt(0)));
					SmartDashboard.putNumber("X", ToteXPos(particles.elementAt(0)));
				} else {
		...
		double ToteXPos(ParticleReport report1)
		{
			return report1.CenterofMassX;
		}
